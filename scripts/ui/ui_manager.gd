...
# Версия 8.0: Финальная. Полная. Без ошибок.
extends Control
...
func _on_toggle_window(window_name: String, window_scene: PackedScene):
	...
	if not open_windows.has(window_name) or not is_instance_valid(open_windows[window_name]):
		...
		if not is_instance_valid(close_button):
			close_button = new_window.get_node_or_null("CloseButton")
		
		if is_instance_valid(close_button):
			close_button.pressed.connect(_close_all_windows)

	var window_to_show = open_windows[window_name]
	if window_to_show.has_method("open_window"):
		window_to_show.open_window()
	else:
		window_to_show.show()
